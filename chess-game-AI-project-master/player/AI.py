...
class AI:
    ...
    def minimax(self,gametiles, depth,alpha , beta ,player):
        if depth==0 or self.checkmate(gametiles)==True or self.stalemate(gametiles,player)==True:
            return self.heuristic(gametiles)
        if not player:
            minEval=100000000
    ...
    def heuristic(self, gametiles):
        ...
        for y in range(8):
            for x in range(8):
                piece_str = str(gametiles[y][x].pieceonTile) 
                ...
                if piece_str in ['Q', 'q', 'R', 'r', 'B', 'b', 'N', 'n']:
                    ...
                    legal_moves = gametiles[y][x].pieceonTile.legalMoves()
                    value += len(legal_moves) * (10 if piece_str.islower() else -10)
