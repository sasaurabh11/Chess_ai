...
from evalution.accuracy_checker import ChessAccuracyChecker
import copy
import chess  # For handling FEN
...
pygame.display.set_caption("CodAI - chess")
...
accuracy_checker = ChessAccuracyChecker(ai, STOCKFISH_PATH)
...
text = font.render(' CodAI - Chess ', True, orange, blue)  
text1 = font.render('AI', True, green)
text2 = font.render('2 player', True, purple)
text3 = font.render('Black won by checkmate', True, green)
text4 = font.render('White won by checkmate', True, green)
text5 = font.render('Stalemate', True, light_gray)
text6 = font.render(' Made by : Saurabh ', True, purple)
    ...
    while not quitgame:
        for event in pygame.event.get():
            ...
            if not turn%2==0 and promotion==False:
                ...
                # game_moves = [
                #     ("rnbqkb1r/pppppppp/5n2/8/8/5N2/PPPPPPPP/RNBQKB1R w KQkq - 0 1", 'e2e4'),  # Example FEN and AI move in UCI
                #     ("rnbqkbnr/pppp1ppp/8/4p3/8/5N2/PPPPPPPP/RNBQKB1R w KQkq - 1 2", 'e4e5')
                # ]
                fen = chessBoard.get_fen()
                ai_move_uci = f"{chr(97 + x)}{8 - y}{chr(97 + m)}{8 - n}"  # Convert to UCI format
                game_moves = [(fen, ai_move_uci)]

                accuracy_score = accuracy_checker.check_accuracy(game_moves)
                print(f"Accuracy of AI's move: {accuracy_score} %")
